...
public class ASMUtils {
    ...
    public static boolean injectInsns(MethodNode method, InsnPredicate[] matchers,
        Supplier<AbstractInsnNode[]> toInject) {
    ...
    public static boolean removeInsns(MethodNode method, InsnPredicate[] matchers) {
        return findInsns(method, matchers, (first, last, nodes) -> { removeInsns(method, nodes); });
    }
    ...
    public static InsnPredicate isGetStatic(@Nullable String owner, @Nullable String desc, @Nullable String name) {
        return (method, insn) -> insn.getOpcode() == Opcodes.GETSTATIC && insn instanceof FieldInsnNode node
    ...
    public static InsnPredicate isPutStatic(@Nullable String owner, @Nullable String desc, @Nullable String name) {
        return (method, insn) -> insn.getOpcode() == Opcodes.PUTSTATIC && insn instanceof FieldInsnNode node
            && (owner == null || owner.equals(node.owner))
            && (desc == null || desc.equals(node.desc))
            && (name == null || name.equals(node.name));
    }
    ...
    public static InsnPredicate isGetVar(@Nullable String desc, @Nullable String name) {
        return (method, insn) -> {
            if (insn.getOpcode() == Opcodes.ALOAD && insn instanceof VarInsnNode node) {
                ...
                return var != null && (desc == null || desc.equals(var.desc))
                    && (name == null || name.equals(var.name));
            } else {
                return false;
            }
        };
    ...
    public static InsnPredicate isInvokeStatic(@Nullable String owner, @Nullable String desc, @Nullable String name) {
        return (method, insn) -> insn.getOpcode() == Opcodes.INVOKESTATIC && insn instanceof MethodInsnNode node
            && (owner == null || owner.equals(node.owner))
            && (desc == null || desc.equals(node.desc))
            && (name == null || name.equals(node.name))
            && node.itf == false;
    ...
    public static InsnPredicate isInvokeInterface(@Nullable String owner, @Nullable String desc,
        @Nullable String name) {
        return (method, insn) -> insn.getOpcode() == Opcodes.INVOKEINTERFACE && insn instanceof MethodInsnNode node
            && (owner == null || owner.equals(node.owner))
            && (desc == null || desc.equals(node.desc))
            && (name == null || name.equals(node.name))
            && node.itf == true;
    ...
    public static InsnPredicate isInvoke(@Nullable String owner, @Nullable String desc, @Nullable String name) {
        return (method, insn) -> insn instanceof MethodInsnNode node && (owner == null || owner.equals(node.owner))
            && (desc == null || desc.equals(node.desc))
            && (name == null || name.equals(node.name));
    }

    public static InsnPredicate isNew(@Nullable String desc) {
        return (method, insn) -> insn.getOpcode() == Opcodes.NEW && insn instanceof TypeInsnNode node
            && (desc == null || desc.equals(node.desc));
    }

    public static InsnPredicate isInit(@Nullable String owner, @Nullable String desc) {
        return (method, insn) -> insn.getOpcode() == Opcodes.INVOKESPECIAL && insn instanceof MethodInsnNode node
            && (owner == null || owner.equals(node.owner))
    ...
    public static InsnPredicate isIf(LabelNode label) {
        return (method, insn) -> insn.getOpcode() == Opcodes.IFEQ && insn instanceof JumpInsnNode node
    ...
    public static void debug(AbstractInsnNode insn, @Nullable List<LocalVariableNode> vars) {
        if (insn instanceof MethodInsnNode method) {
            ...
            for (int i = 0; i < (frame.local == null ? 0 : frame.local.size()); i++) {
                String varName = vars == null ? "[unknown]"
                    : i >= vars.size() ? "[illegal var index]" : vars.get(i).name;
